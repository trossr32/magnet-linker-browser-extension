                ...
                getSettings(function(settings) {
                    ...
                    setSettings(settings, function () {
                        ...
                        if (settings.pageHasMagnets) {
                            let bootstrapCss = browser.tabs.insertCSS(null, { file: 'content/css/bootstrap.tw.min.css' });

                            bootstrapCss.then(() => {
                                let contentCss = browser.tabs.insertCSS(null, { file: 'content/css/content_script.css' });

                                contentCss.then(() => {
                                    let bootstrapJs = browser.tabs.executeScript(null, { file: 'content/js/bootstrap.micro.min.js' });

                                    bootstrapJs.then(() => {
                                        browser.tabs.executeScript(null, { file: 'content/js/content_script.js' });
                                    });                                        
                                });                                    
                            });                                
                        }
...
var getSettings = function(callback) {
    let s = browser.storage.local.get({ 'magnetLinkerSettings': defaultSettings });
